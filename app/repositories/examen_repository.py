...
from app.models.exams import ExamModel
from app.schemas.exams import ExamSchema

class ExamenRepository:
    def __init__(self, db: AsyncSession):
        self.db = db

    async def create_examen(self, examen_data: ExamSchema, user_uid: str, username: str):
    ...
    async def get_exams_by_user_uid(self, user_uid: str):
        # Obtener los exámenes de un usuario específico
        query = select(ExamModel).where(ExamModel.user_uid == user_uid)
        result = await self.db.execute(query)
        return result.scalars().all()

    async def get_examen_by_id(self, examen_id: int):
        # Obtener un examen por su ID
        query = select(ExamModel).where(ExamModel.id_examen == examen_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def delete_examen(self, examen_id: int):
        # Eliminar un examen
        query = delete(ExamModel).where(ExamModel.id_examen == examen_id)
        result = await self.db.execute(query)
        await self.db.commit()
        return result.rowcount  # Devuelve cuántos registros fueron eliminados

    async def update_examen(self, examen_id: int, examen_data: ExamSchema):
        # Actualizar un examen
        query = (
            update(ExamModel)
            .where(ExamModel.id_examen == examen_id)
            .values(
                exam_type=examen_data.exam_type,
                res_1=examen_data.res_1,
                res_2=examen_data.res_2,
